...
public class CajeroServiceImpl implements CajeroService {
	...
	public void ingresar(int numCuenta, double cantidad) {
		...
		this.movimientos.addMovimiento(new Movimiento(0, cantidad, new Date(), "ingreso", cuenta));
	}
	...
	public boolean extraccion(int numCuenta, double cantidad) {
		...
		if(resta < 0)
			...
			this.movimientos.addMovimiento(new Movimiento(0,cantidad, new Date(),"extraccion", cuenta));
	...
	@Override
	public List<Cliente> titularesCuenta(int numeroCuenta) {
		return this.cuentas.obtenerClientes(numeroCuenta);
	}
